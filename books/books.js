...
function parseSearchQuery(query) {
  if (!query.trim()) return { terms: [], operators: [], notTerms: [] };
  // クエリをトークンに分割（スペース区切り）
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token === 'and' || token === '&&') operators.push('AND');
    else if (token === 'or' || token === '||') operators.push('OR');
    else if (token === 'not' || token === '!') {
      if (i + 1 < tokens.length) { // NOTの次のトークンをnotTermsに追加
        notTerms.push(tokens[i + 1]);
        i++; // 次のトークンをスキップ
      }
    } else terms.push(tokens[i]); // 通常の検索語
  }
...
function evaluateQuery(book, query) {
  ...
  if (terms.length === 1) return matchBook(book, terms[0]);
  // 複数の検索語がある場合
...
function filterBooks(books, searchTerm = '', categoryFilter = '', statusFilter = '') {
  return books.filter(book => {
    ...
    const matchesSearch = !searchTerm || (() => {
      return evaluateQuery(book, parseSearchQuery(searchTerm));
    })();
